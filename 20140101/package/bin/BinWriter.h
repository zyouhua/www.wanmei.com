...
namespace std {
	...
	class BinWriter : public noncopyable
	{
	public:
	...
	template<class __t>
	void BinWriter::_serialize(__t * nValue, const wchar_t * nName)
	{
	...
	template<class __t>
	void BinWriter::_serialize(list<__t *> nValue, const wchar_t * nName)
	{
	...
	template<class __t>
	void BinWriter::_serialize(shared_ptr<__t> nValue, const wchar_t * nName)
	{
		nValue->_serialize(this);
	}

	template<class __t>
	void BinWriter::_serialize(list<shared_ptr<__t>> nValue, const wchar_t * nName)
	{
		__u16 count_ = static_cast<__u16>(nValue.size());
		mStream.write((char *)(&count_), sizeof(__u16));
		list<shared_ptr<__t>>::iterator it = nValue.begin();
		for ( ; it != nValue.end(); ++it )
		{
			shared_ptr<__t>& t_ = (*it);
			t_->_serialize(this);
		}
	}
